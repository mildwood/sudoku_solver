    ...
    <v-container fluid>
		<v-row class="d-flex" justify="center">
			<v-col class="d-flex justify-center">
				<h4 class="mr-8">Minimum kitöltendő mező: 17</h4>
				<h4>Kitöltött mező: {{filledCellNum}}</h4>
			</v-col>				
		</v-row>
		<v-row class="align-center justify-center mt-4">
			<v-col class="d-flex align-center justify-center" >
				<v-card elevation="8" solo class="pt-3 pb-3 mx-n12 rounded-xl" color="grey darken-3" min-width="750" >
					<v-row justify="space-between" class="align-center ma-0 px-12 mx-5 py-n3" v-for="x in xObj.num" :key="x.uuid">
						<v-col class="d-flex align-center justify-space-around  ma-0 pa-0 blockColumn py-1" v-for="y in yObj.num" :key="y.uuid">
								...
								<v-row justify="space-around" class="align-center justify-space-between" v-for="z in zObj.num" :key="z.uuid">
        							<v-col class="align-center px-n12 cellColumn" v-for="g in gObj.num" :key="g.uuid">
										...
										v-mask="'#'"
					  					...
					  					@input="addCellNum((((x-1)*3)+z)-1,(((y-1)*3)+g)-1)"
		...
		<v-row justify="center" class="d-flex">
			<v-col class="d-flex justify-center">
				<v-btn :disabled="filledCellNum < 17" class="mr-5" @click.prevent="solvePuzzle2">Old meg!</v-btn>
				<v-btn @click.prevent="resetPuzzle">Reset</v-btn>
			</v-col>
		</v-row>
	</v-container>
  </v-form>
...
checkBlock(row,col,num,inputType){
	...
	for (var i=0; i<=8; i++) {
		if (numbers[i].value == num && numbers[i].x !== row && numbers[i].y !== col) {
			...
			// Insert input type
			this.sudoku[row][col].inputType = inputType
			return false;
		}
	}
...
// Check possibilities in a cell
possibleNumbers(row,col,inputType){
	var nums = []
	for(var x=1; x<=9; x++){
		if(this.checkRow(row,col,x,inputType) && this.checkColumn(row,col,x,inputType) && this.checkBlock(row,col,x,inputType)){
			if(row == 3 && col == 0){
				console.log(x)
			}
			nums.push(x)
	...
	if(nums.length === 1){
		if(row == 3 && col == 0){
			console.log("FOUND ONLY ONE NUMBER")
			console.log(nums[0])
		}
		this.sudoku[row][col].value = nums[0]
		this.sudoku[row][col].inputType = inputType
		this.sudoku[row][col].wrong = "false"
		return true;
	}
	else {
		this.sudoku[row][col].possibilities = nums
	}

},

solvePuzzle(){
	// Check if there is no wrong number in the puzzle
	for (var h = 0; h <=8; h++){
		for (var k = 0; k <=8; k++){
			if(this.sudoku[h][k].wrong === 'true'){
				//Break checking if a wrong number found
        		this.$store.commit('setSnackbarMessage', 'Hibás szám a puzzle-be!')
	...
	// Loop trough all unfilled cells
	for(var z = 0; z <=81-this.filledCellNum; z++){
		for(var x = 0; x <=8; x++){
			for(var y = 0; y <=8; y++){
				if(!this.sudoku[x][y].value.length){
					this.possibleNumbers(x,y,'machine')
				}
			}
		}
	}
	...
	//  for (var h = 0; h <=8; h++){
	// 	 for (var k = 0; k <=8; k++){
	// 		 for(const num of this.sudoku[h][k].possibilities){
	// 			 var uniqueNum = true
	// 			 for (var j = 0; j <=8; j++){
	// 				 if(this.sudoku[h][j].possibilities.includes(num)){
	// 					//  Exclude own cell
	// 					 if(j !== k){
	// 						 uniqueNum = false
	// 					 }
	...
	// 			var uniqueNum = true
	// 			 for (var j = 0; j <=8; j++){
	// 				 if(this.sudoku[j][k].possibilities.includes(num)){
	// 					//  Exclude own cell
	// 					 if(j !== h){
	// 						 uniqueNum = false
	// 					 }
	...
	// 			var uniqueNum = true
	// 			// Calculate blockNum
	// 			var blockNum = Math.floor(h/3)*3+Math.floor(k/3)
	// 			// Calculate column
	// 			if(blockNum*3 < 9){
	// 				var column = blockNum*3
	// 			}
	...
	// 			}
	// 			for (var j = 0; j <=2; j++){
	// 				 if(this.sudoku[Math.floor(blockNum/3)*3][column+j].possibilities.includes(num)){
	// 						uniqueNum = false
	// 				if(this.sudoku[Math.floor(blockNum/3)*3+1][column+j].possibilities.includes(num)){
	// 						uniqueNum = false
	// 				 	}
	// 				 if(this.sudoku[Math.floor(blockNum/3)*3+2][column+j].possibilities.includes(num)){
	// 						uniqueNum = false
	// 				 	}	 
	// 				 }
	// 			 }
	...
	// If could not complete than it is more complicated than the resolver
...
// Reset sudoku
resetPuzzle(){
	this.sudoku = this.cleanSudoku
	this.filledCellNum = 0
	localStorage.sudoku = this.cleanSudoku
},
}
}
