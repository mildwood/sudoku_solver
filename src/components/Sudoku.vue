  ...
  <v-form>
    ...
    <v-container fluid>
		...
		<v-row justify="center" class="d-flex">
			<v-col class="d-flex justify-center">
				<v-btn :disabled="filledCellNum < 17" class="mr-5" @click.prevent="solvePuzzle">Old meg!</v-btn>
				<v-btn class="mr-5" @click.prevent="resetPuzzle">Reset</v-btn>
				<v-btn disabled class="mr-5" @click.prevent="rollBack">Roll Back
					<v-icon class="ml-2">
						mdi-arrow-u-left-top
					</v-icon>
				</v-btn>
				<v-btn :disabled='parseInt(filledCellNum) > 80' @click.prevent="hint">Hint
					<v-icon class="ml-2">
  ...
  methods: {
	...
	if(this.sudoku[row][col].value.length){
		if(!this.sudoku[row][col].inputType.length){
			this.checkRow(row,col,this.sudoku[row][col].value,this.inputType)
			this.checkColumn(row,col,this.sudoku[row][col].value,this.inputType)
			this.checkBlock(row,col,this.sudoku[row][col].value,this.inputType)
		}
		else{
...
checkBlock(row,col,num,inputType){
	...
	// Insert input type
	this.sudoku[row][col].inputType = this.inputType
			...
			for (var r = 0; r <=8; r++){
				for (var c = 0; c <=8; c++){
					if(!this.sudoku[r][c].value.length){
						...
						this.sudoku[r][c].inputType = this.inputType
					}
				}
			}
		}
		// Switch back to human mode
		this.inputType = "human"
		this.countFilledCells()
},
...
hint(){
	...
	for (var r = 0; r <=8; r++){
		...
		if(!result){
			...
			this.sudoku[_row][_col].inputType = this.inputType
		}
		// Switch back to human mode
		this.inputType = "human"
		this.countFilledCells()
...
resetPuzzle(){
	...
	for(var x=0; x<=8; x++){
		for(var y=0; y<=8; y++){
			tempSudoku = tempSudoku + x + "," + y + "," + "" + "," + "" + "," + "false" + ";"
		}
	}
	localStorage.sudoku = tempSudoku
	},
...
rollBack(){
	...
	this.filledCellNum = 0
	localStorage.sudoku = this.sudoku
	},
}
}
